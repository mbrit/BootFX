...
{
	/// <summary>
	/// Summary description for DataDialog.
	/// </summary>
	public class DataDialog : BaseForm, IEntityDataControl
	{
		/// <summary>
		/// Private field to support <c>SaveChangesOnApply</c> property.
		/// </summary>
		private bool _saveChangesOnApply = true;
		...
		/// <summary>
		/// Gets or sets the savechangesonapply
		/// </summary>
		[Browsable(true), Category("Behavior"), DefaultValue(true)]
		public bool SaveChangesOnApply
		{
			get
			{
				return _saveChangesOnApply;
			}
			set
			{
				// check to see if the value has changed...
				if(value != _saveChangesOnApply)
				{
					// set the value...
					_saveChangesOnApply = value;
				}
			}
		}
		...
		{
			// TODO: handle what happens here if we have multiple items, which we should virtually never have with a dialog...
			...
			EntityType.Persistence.SaveChanges(this.CurrentEntity);
		}
	}
}
