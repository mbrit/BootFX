...
namespace BootFX.Common.CodeGeneration
{
	...
	public class EntityGenerator
	{
        ...
        public virtual GeneratedTypeDeclaration GetEntityClass(SqlTable table, EntityGenerationContext context)
		{
			...
			CodeDomHelper.AddSummaryComment(type, string.Format("Defines the entity type for '{0}'.", table.NativeName));
        ...
        public GeneratedTypeDeclaration GetEntityWsClass(SqlTable table, EntityGenerationContext context)
		{
			...
			CodeDomHelper.AddSummaryComment(type, string.Format("Defines the webservice entity type for '{0}'.", table.Name));
			type.CustomAttributes.Add(new CodeAttributeDeclaration("Serializable"));
        ...
        public GeneratedTypeDeclaration GetEntityWsBaseClass(SqlTable table, EntityGenerationContext context)
		{
			...
			CodeDomHelper.AddSummaryComment(type, string.Format("Defines the base webservice entity type for '{0}'.", table.Name));
			type.CustomAttributes.Add(new CodeAttributeDeclaration("Serializable"));
		...
		private CodeMemberField CreateFieldForWsClass(SqlTable table, SqlColumn column)
		{
			...
			CodeDomHelper.AddSummaryComment(field, string.Format("Gets or sets the value for '{0}'.", column.Name));
			CodeDomHelper.AddRemarksComment(field, string.Format("This field maps to the '{0}' column.", column.NativeName));
        ...
        public virtual GeneratedTypeDeclaration GetEntityCollectionClass(SqlTable table, EntityGenerationContext context)
		{
			...
			CodeTypeDeclaration type = new CodeTypeDeclaration(className);
			type.TypeAttributes = table.GetTypeAttributes(false, context);
			CodeDomHelper.AddSummaryComment(type, string.Format("Defines the collection for entities of type <see cref=\"{0}\"/>.", 
        ...
        public virtual GeneratedTypeDeclaration GetEntityCollectionBaseClass(SqlTable table, EntityGenerationContext context)
		{
			...
			CodeDomHelper.AddSummaryComment(type, string.Format("Defines the base collection for entities of type <see cref=\"{0}\"/>.", 
				this.GetEntityClassName(table)));
			type.CustomAttributes.Add(new CodeAttributeDeclaration("Serializable"));
		...
		private CodeMemberMethod GetCollectionAddRangeMethod(SqlTable table, CodeParameterDeclarationExpression parameter)
		{
			...
			CodeDomHelper.AddSummaryComment(method, string.Format("Adds a range of <see cref=\"{0}\"/> instances to the collection.", this.GetEntityClassName(table)));
		...
		private CodeMemberMethod GetCollectionAddMethod(SqlTable table)
		{
			...
			CodeDomHelper.AddSummaryComment(method, string.Format("Adds a <see cref=\"{0}\"/> instance to the collection.", this.GetEntityClassName(table)));
		...
		private CodeMemberProperty GetCollectionIndexer(SqlTable table)
		{
			...
			CodeDomHelper.AddSummaryComment(property, "Gets or sets the item with the given index.");
		...
		public virtual GeneratedTypeDeclaration GetEntityBaseClass(SqlTable table, EntityGenerationContext context)
		{
			...
			CodeTypeDeclaration type = new CodeTypeDeclaration(className);
			CodeDomHelper.AddSummaryComment(type, string.Format("Defines the base entity type for '{0}'.", table.NativeName));
			type.CustomAttributes.Add(new CodeAttributeDeclaration("Serializable"));
		...
		protected virtual CodeMemberMethod GetCreateFilterStaticMethod(SqlTable table, EntityGenerationContext context)
		{
			...
			CodeDomHelper.AddSummaryComment(method, string.Format("Creates a SqlFilter for an instance of '{0}'.", table.Name));
        ...
        public GeneratedTypeDeclaration GetDtoBaseClass(SqlTable table, EntityGenerationContext context)
        {
            ...
            CodeDomHelper.AddSummaryComment(type, string.Format("Defines the base DTO object for entity type '{0}'.", table.Name));
		...
		public GeneratedTypeDeclaration GetDtoClass(SqlTable table, EntityGenerationContext context)
		{
            ...
            CodeDomHelper.AddSummaryComment(type, string.Format("Defines the DTO object for entity type '{0}'.", table.Name));
            //type.TypeAttributes = table.GetTypeAttributes(false, context);
            ...
            //CodeDomHelper.AddSummaryComment(type,wsDescription);
		...
		private CodeMemberMethod GetToStringMethod(SqlTable table)
		{
			...
			foreach(SqlColumn column in table.Columns)
			{
				if(string.Compare(column.Name, "Name", true, System.Globalization.CultureInfo.InvariantCulture) == 0)
				{
					...
					CodeDomHelper.AddSummaryComment(toString, string.Format("Returns the value in the '{0}' property.", column.Name));
		...
		private CodeMemberMethod CreateOnFieldChangedMethod(SqlColumn column)
		{
			...
			CodeDomHelper.AddSummaryComment(method, string.Format("Raises the <see cref=\"{0}Changed\"/> event.", column.Name));
		...
		private CodeMemberEvent CreateFieldChangedEvent(SqlColumn column)
		{
			...
			CodeDomHelper.AddSummaryComment(changedEvent, string.Format("Raised when <see cref=\"{0}\"/> is changed.", column.Name));
...
//				method.Attributes = MemberAttributes.Public | MemberAttributes.Static;
			...
			if(additionalColumns.Length == 0)
				CodeDomHelper.AddSummaryComment(method, string.Format("Get all of the child '{0}' entities.", this.GetEntityClassName(childTable)));
			else
				CodeDomHelper.AddSummaryComment(method, string.Format("Get the child '{0}' entities constrained by the given parameters.", this.GetEntityClassName(childTable)));
		...
		protected virtual CodeMemberProperty GetChildToParentLinkProperty(SqlTable table, SqlChildToParentLink link, EntityGenerationContext context)
		{
			...
			CodeDomHelper.AddSummaryComment(property, string.Format("Gets the link to '{0}'.", this.GetEntityClassName(link.ParentTable)));
			CodeDomHelper.AddRemarksComment(property, string.Format("This property maps to the '{0}' constraint.", link.NativeName));
		...
		private void AddMethod(CodeTypeDeclaration type, CodeMemberMethod method, ArrayList signatures, string remarks)
		{
			...
			if(signatures.Contains(signature) == false)
			{
				...
				if(remarks != null && remarks.Length > 0)
					CodeDomHelper.AddRemarksComment(method, remarks);
			}
		}
		...
		protected virtual CodeMemberProperty GetFieldAccessProperty(SqlTable table, SqlColumn column, EntityGenerationContext context)
		{
			...
			CodeDomHelper.AddSummaryComment(property, string.Format("Gets or sets the value for '{0}'.", column.Name));
			CodeDomHelper.AddRemarksComment(property, string.Format("This property maps to the '{0}' column.", column.NativeName));
			...
			// enum?
			if(column.HasEncryptionKeyName)
				property.CustomAttributes.Add(new CodeAttributeDeclaration("Encrypted", new CodeAttributeArgument(new CodePrimitiveExpression(column.EncryptionKeyName))));
		...
		private CodeExpression GetEntityFlagsExpression(SqlColumn column)
		{
			...
			expression = this.CombineEntityFlags(column, expression, EntityFieldFlags.PartitionId);
		...
		private CodeExpression CombineEntityFlags(SqlColumn column, CodeExpression flagsExpression, EntityFieldFlags flag)
		{
			...
			// combine...
			return CodeDomHelper.CombineFlagsExpressions(flagsExpression, newFlagExpression);
		}
		...
		private CodeConstructor CreateNoOpConstructor(MemberAttributes attributes)
		{
			...
			CodeDomHelper.AddSummaryComment(constructor, "Constructor.");
		...
		private CodeConstructor CreateDeserializationConstructor()
		{
			...
			CodeDomHelper.AddSummaryComment(constructor, "Deserialization constructor.");
		...
		private CodeMemberMethod CreateToWsEntityMethod(SqlTable table)
		{
			...
			CodeDomHelper.AddSummaryComment(toWsEntityMethod, string.Format("Gets the 'Ws{0} of this entity'.", table.Name));
		...
		private CodeMemberMethod CreateToWsEntityArrayMethod(SqlTable table)
		{
			...
			CodeDomHelper.AddSummaryComment(toWsArrayMethod, string.Format("Gets a 'Ws{0}' array of items for this collection.", table.Name));
		...
		private CodeMemberMethod GetSetPropertyMethod(SqlTable table, MemberTarget target)
		{
			...
			CodeDomHelper.AddSummaryComment(method, string.Format("Sets properties on an instance of '{0}'.", table.Name));
		...
		private CodeMemberMethod GetCreateInstanceMethod(SqlTable table, MemberTarget target)
		{
			...
			CodeDomHelper.AddSummaryComment(method, string.Format("Creates and instance of '{0}'.", table.Name));
		...
		private CodeMemberMethod GetUpdateInstanceMethod(SqlTable table)
		{
			...
			CodeDomHelper.AddSummaryComment(method, string.Format("Updates an instance of '{0}'.", table.Name));
		...
		protected virtual CodeMemberMethod GetGetAllMethod(SqlTable table, MemberTarget target, EntityGenerationContext context)
		{
			...
			CodeDomHelper.AddSummaryComment(method, string.Format("Get all <see cref=\"{0}\"/> entities.", this.GetEntityClassName(table)));
		...
		private CodeMemberMethod GetGetByIdWrapper(SqlTable table, bool includeOnNotFound)
		{
			...
			CodeDomHelper.AddSummaryComment(method, "Gets the entity with the given ID.");
			CodeDomHelper.AddRemarksComment(method, "This method checks to see if the entity type supports caching and, if so, defers to the cache first.");
		...
		private CodeMemberMethod GetGetByIdUsingCacheMethod(SqlTable table)
		{
			...
			method.ReturnType = new CodeTypeReference(this.GetEntityClassName(table));
			method.Attributes = MemberAttributes.Private | MemberAttributes.Static;
			CodeDomHelper.AddSummaryComment(method, "Gets the entity with the given ID using the cache for this entity type.");
			...
			SqlColumn[] columns, bool returnsCollection, bool addOnNotFound, bool deferToNotFound, MemberTarget target)
		{
			...
			// xml...
			CodeDomHelper.AddSummaryComment(method, summary);
			CodeDomHelper.AddRemarksComment(method, remarks);
			CodeDomHelper.AddFxComment(method, string.Format("CreateEntityFilterEqualToMethod - {0}", table));
			...
			switch(target)
			{
				...
				case MemberTarget.Concrete | MemberTarget.WebMethod:
					...
					if(addOnNotFound)
					{
						...
						string resultsName = CodeDomHelper.GetNextIdentifierName("results", checkNames);
						if(resultsName == null)
		...
		public Assembly CompileToAssembly(string namespaceName, SqlTable[] tables, EntityGenerationContext context)
		{
			...
			try
			{
				...
				// compile it...
				return CodeDomHelper.Compile(unit, this.CompilationOptions.CompilerOptions, Language.CSharp);
			}
			finally
			{
		...
		protected virtual CodeMemberMethod[] GetSearchMethods(SqlTable table, MemberTarget target, EntityGenerationContext context)
		{
			...
			method.Name = "Search";
			CodeDomHelper.AddSummaryComment(method, string.Format("Searches for <see cref=\"{0}\"/> items with the given terms.", this.GetEntityClassName(table)));
		...
		public GeneratedTypeDeclaration GetEntityClass(SqlProcedure proc, EntityGenerationContext context)
		{
			...
			CodeDomHelper.AddSummaryComment(type, string.Format("Provides access to the <c>{0}</c> procedure.", proc.Name));
			type.BaseTypes.Add(this.GetEntityBaseClassName(proc));

			// cons...
			CodeConstructor cons = new CodeConstructor();
			type.Members.Add(cons);
			CodeDomHelper.AddSummaryComment(cons, "Creates a new instance of the object.");
		...
		public GeneratedTypeDeclaration GetEntityBaseClass(SqlProcedure proc, EntityGenerationContext context)
		{
			...
			CodeDomHelper.AddSummaryComment(type, string.Format("Base class for providing access to the <c>{0}</c> procedure.", proc.Name));
			CodeDomHelper.AddRemarksComment(type, string.Format("SQL: {0}", proc.Body));
			...
			CodeConstructor cons = new CodeConstructor();
			CodeDomHelper.AddSummaryComment(cons, "Constructs the object and configures parameters for the procedure.");
			type.Members.Add(cons);
		...
		private CodeMemberProperty GetParameterAccessProperty(SqlStatementParameter param)
		{
			...
			CodeDomHelper.AddSummaryComment(prop, string.Format("Provides access to the <c>{0}</c> parameter, type=<c>{1}</c>, default=<c>{2}</c>, direction=<c>{3}</c>.", 
